...
export const AnalysisResultCard: React.FC<Props> = ({ candidate, onToggleSelection, onDelete, index = 0 }) => {
  ...
  useEffect(() => {
    if (candidate.result?.matchScore) {
        const timer = setTimeout(() => {
            setProgress(candidate.result!.matchScore * 10);
        }, 300 + (index * 100));
  ...
  if (candidate.status === CandidateStatus.PENDING) {
    return (
      ...
      >
        ...
        <div className="flex items-center gap-3 relative z-20" onMouseDown={(e) => e.stopPropagation()}>
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider group-hover:text-slate-600">Aguardando</span>
            {onDelete && (
                !confirmDelete ? (
                    <button onClick={handleDeleteClick} className="p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-md transition-all cursor-pointer z-30 relative pointer-events-auto">
                        <Trash2 className="w-4 h-4 pointer-events-none" />
                    </button>
                ) : (
                    <div className="flex items-center gap-1 bg-red-50 rounded-md border border-red-100 p-1 animate-fade-in">
  ...
  if (candidate.status === CandidateStatus.ERROR || !candidate.result) {
    return (
      <div className="bg-red-50 border-2 border-red-200 rounded-xl p-4 flex items-center justify-between text-red-600 mb-3 animate-slide-up relative shadow-sm" onMouseLeave={handleMouseLeave}>
        ...
        {onDelete && (
             !confirmDelete ? (
                <button onClick={handleDeleteClick} className="p-1.5 hover:bg-red-100 rounded-md transition-colors cursor-pointer relative z-20 pointer-events-auto">
                    <Trash2 className="w-4 h-4 pointer-events-none" />
                </button>
             ) : (
  ...
  return (
    <div 
      ...
      onMouseLeave={handleMouseLeave}
    >
      <div className="absolute inset-0 z-0 cursor-pointer" onClick={handleCardClick} title="Clique para ver detalhes" />
          ...
          <div className="flex items-center gap-5 flex-1 min-w-0">
              <div className="relative w-16 h-16 flex items-center justify-center shrink-0">
                  <svg className="absolute top-0 left-0 w-full h-full transform -rotate-90" viewBox="0 0 52 52">
                      ...
                      <circle cx="26" cy="26" r={radius} fill="none" stroke={scoreColor} strokeWidth="4" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} strokeLinecap="round" style={{ transition: 'stroke-dashoffset 1s ease-out' }} />
                  </svg>
                  <span className={`text-xl font-black tracking-tighter z-10 ${isHighMatch ? 'text-black' : ''}`} style={!isHighMatch ? { color: scoreColor } : {}}>{result.matchScore}</span>
                  <div className="absolute -top-1 -right-1 bg-black text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full shadow-sm border border-white">#{index + 1}</div>
              </div>
