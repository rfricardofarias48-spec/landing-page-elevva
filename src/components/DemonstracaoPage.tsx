...
import { Bot, Calendar, FileText, BarChart2, Zap, MessageSquare, Check, ArrowRight, Star, ChevronRight, Play, Pause, Volume2, Volume1, VolumeX } from 'lucide-react';
    ...
    <div className="bg-white flex" style={{ minHeight: '680px' }}>
      ...
      <div className="w-64 border-r border-slate-100 bg-white flex flex-col shrink-0 hidden md:flex">
        <div className="h-20 flex items-center justify-center border-b border-slate-100 px-5">
          <img src="https://ik.imagekit.io/xsbrdnr0y/Elevva_Logo_Black.png" alt="Elevva" className="h-9 w-auto object-contain" />
        </div>
        <nav className="p-4 space-y-1.5 flex-1">
          {[
            ...
            <div key={label} className={`w-full flex items-center px-4 py-3 rounded-xl text-sm font-bold ${active ? 'bg-black text-white' : 'text-slate-400'}`}>
              ...
              {[
                ...
                <div key={name} className="flex items-center justify-between py-3 border-b border-slate-50 last:border-0">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-sm font-black text-slate-600 shrink-0">{name.charAt(0)}</div>
                    <div>
...
export function DemonstracaoPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 selection:bg-[#65a30d] selection:text-white" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <section className="max-w-6xl mx-auto px-6 pt-20 pb-6 text-center"
        style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(16px)', transition: 'all 0.7s ease' }}>
        ...
        {/* Tabs de demo — estilo Hero-195 */}
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-slate-100 border border-slate-200 p-1 rounded-2xl h-auto gap-1 flex-wrap justify-center">
            {demoTabs.map(({ id, label, icon: Icon }) => (
              <TabsTrigger
                key={id}
                value={id}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all duration-200 text-slate-500 data-[state=active]:bg-black data-[state=active]:text-white data-[state=active]:shadow-md"
              >
                <Icon className="w-4 h-4 data-[state=active]:text-[#65a30d]" />
                {label}
              </TabsTrigger>
            ))}
          </TabsList>
      ...
      {/* ── TELA COM BORDER BEAM ── */}
      <section className="px-4 md:px-8 pb-24 pt-4"
        style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(24px)', transition: 'all 0.7s ease 0.2s' }}>

        {/* Frame com BorderBeam */}
        <div className="relative rounded-[2rem] border border-slate-200 shadow-[0px_40px_120px_rgba(0,0,0,0.12)] overflow-hidden max-w-[1400px] mx-auto">

          {/* BorderBeam girando nas cores do Elevva */}
          <BorderBeam size={400} duration={8} colorFrom="#65a30d" colorTo="#a3e635" borderWidth={2} />
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {features.map(({ icon: Icon, tag, title, description, stat, statLabel }) => (
              <div key={title}
                className="relative bg-white rounded-[2rem] border border-slate-100 p-8 shadow-[0px_4px_20px_rgba(0,0,0,0.02)] hover:shadow-[0px_8px_32px_rgba(0,0,0,0.06)] transition-all duration-300 group overflow-hidden">
                <BorderBeam size={250} duration={12} colorFrom="#65a30d" colorTo="#a3e635" borderWidth={1.5} />
                <div className="flex items-start justify-between mb-5">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 border border-slate-100 rounded-full px-3 py-1">{tag}</span>
                </div>
      ...
      <section className="py-24 max-w-6xl mx-auto px-6">
        <div className="max-w-xl mb-14">
          ...
          <h2 className="text-4xl font-black tracking-tighter text-slate-900 leading-tight">Quem já usa o Elevva</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {testimonials.map(({ name, role, text, initials }) => (
            <div key={name} className="relative bg-white rounded-[2rem] border border-slate-100 p-7 shadow-[0px_4px_20px_rgba(0,0,0,0.02)] flex flex-col gap-5 overflow-hidden">
              <BorderBeam size={200} duration={14} colorFrom="#65a30d" colorTo="#a3e635" borderWidth={1} />
              <p className="text-sm text-slate-600 leading-relaxed font-medium flex-1">"{text}"</p>
              <div className="flex items-center gap-3 pt-4 border-t border-slate-50">
                <div className="w-9 h-9 rounded-full bg-black text-white flex items-center justify-center text-xs font-black shrink-0">{initials}</div>
                <div>
      ...
      {/* ── CTA FINAL ── */}
      <section className="relative bg-black mx-4 md:mx-6 mb-8 rounded-[2rem] py-16 px-8 text-center overflow-hidden">
        <BorderBeam size={500} duration={6} colorFrom="#65a30d" colorTo="#a3e635" borderWidth={2} />
      ...
      <footer className="max-w-6xl mx-auto px-6 py-8 flex flex-col md:flex-row items-center justify-between gap-3">
        <img src="https://ik.imagekit.io/xsbrdnr0y/Elevva_Logo_Black.png" alt="Elevva" className="h-6 w-auto object-contain opacity-40" />
